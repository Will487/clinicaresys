...
<div class="container-fluid">
    <!-- Page Heading -->
    <h1 class="h3 mb-2 text-gray-800">Profissional de Saúde</h1>
            ...
            <table id="datatable" class="display">
                <thead>
                    <tr>
                        ...
                        <th align="center">Carteira Profissional</th>
                ...
                <tbody>
                    <c:forEach var="profsaude" items="${profsaudes}">                    
                        <tr>
                            <td align="center">${profsaude.idProfsaude}</td>  
                            <td align="center">${profsaude.cartprof}</td>
                            <td align="center">${profsaude.nome}</td>
                            <td align="center">${profsaude.cpfCnpj}</td>
                       <td align="center">${profsaude.cidade.nomeCidade}-${profsaude.cidade.estado.siglaEstado}</td>
                            <td align="center">
                                <a href="#modaladicionar" class="btn btn-success adicionar" data-toggle="modal" 
                                   data-id="" onclick="setDadosModal(${profsaude.idProfsaude})">
                                    <i class="fas fa-plus fa-fw"></i> Alterar </a>
                            </td>
                            <td align="center">
                                <a href="#" onclick="deletar(${profsaude.idProfsaude})">
                                    <button class="btn 
                                          <c:out value="${profsaude.situacao == 'A' ? 'btn-danger': 'btn-success'}"/>">
                                        <i class="fas fa-fw 
                                           <c:out value="${profsaude.situacao == 'A' ? 'fa-times' : ' fa-plus'}"/>"></i>
                                        <Strong>
                                            <c:out value="${profsaude.situacao == 'A' ? 'Inativar': 'Ativar'}"/>
                                        </Strong>
                                    </button></a>
                            </td>
                        </tr>                
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <input class="form-control" type="text" name="idpessoa" id="idpessoa" value="" 
                               readonly="readonly"/>
                        <input class="form-control" type="text" name="idprofsaude" id="idprofsaude" 
                                ...
                                <select class="form-control" name="idestado" id="idestado" 
                                    ...
                                    <c:forEach var="estado" items="${estados}">
                                        <option value="${estado.idEstado}"
                                                ${profsaude.cidade.estado.idEstado == estado.idEstado ? "selected" : ""}>
                                            ${estado.nomeEstado}
                                        </option>
                                    </c:forEach>
                                </select>
                                ...
                                <select class="form-control" name="idcidade" id="idcidade" required>
                                    ...
                                    <c:forEach var="cidade" items="${cidades}">
                                        <option value="${cidade.idCidade}"
                                                ${profsaude.cidade.idCidade == cidade.idCidade ? "selected" : ""}>
                                            ${cidade.nomeCidade} 
                                        </option>
                                    </c:forEach>
                                </select>
                        ...
                        <div class="form-line row">
                            ...
                            <div class="col-sm">
                                ...
                                <select class="form-control" name="permitelogin" id="permitelogin">
                                    <option value="N" ${profsaude.permiteLogin == 'N' ? "selected" : ""}>Não</option>
                                    <option value="S" ${profsaude.permiteLogin == 'S' ? "selected" : ""}>Sim</option>       
                                </select>    
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" type="text" name="cartprof" id="cartprof"/>
                    </div>
...
<script>
    ...
    function setDadosModal(valor) {
        ...
        document.getElementById('idprofsaude').value = valor;
        var idCli = valor;
        if (idCli != "0"){
            //existe profsaude para buscar (alteração)
            $.getJSON('ProfsaudeCarregar', {idProfsaude: idCli}, function(respostaServlet){
                console.log(respostaServlet);
                var id = respostaServlet.idProfsaude;
                if(id != "0"){
                    $('#idpessoa').val(respostaServlet.idPessoa);
                    $('#idprofsaude').val(respostaServlet.idProfsaude);
                    $('#situacao').val(respostaServlet.situacao);
                    $('#cartprof').val(respostaServlet.cartprof);
    ...
    function carregarPessoa(v) {
        ...
        $(document).ready(function () {
            $.getJSON('PessoaBuscarCpfCnpj', {cpfcnpjpessoa: idM, tipopessoa: tipoPessoa}, function (respostaCli) {
                ...
                //var id = respostaCli.idProfsaude;
                if (respostaCli != null)
                {
                    $('#idprofsaude').val(respostaCli.idProfsaude);
                    $('#permitelogin').val(respostaCli.permiteLogin);
                    $('#situacao').val(respostaCli.situacao);
                    $('#cartprof').val(respostaCli.cartprof);
                    ...
                    //se não encontrou profsaude busca por pessoa somente
                    tipoPessoa = 'pessoa';
    ...
    function deletar(codigo){
        ...
        Swal.fire({
            ...
            text: "Você deseja realmente inativar/ativar o profsaude?",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'post',
                    url: '${pageContext.request.contextPath}/ProfsaudeExcluir',
                    data:{
                        idProfsaude: id
                    },
                    success:
                        function(data){
                            if(data == 1){
                                Swal.fire({
                                    ...
                                    icon: 'success',
                                    title: 'Sucesso',
                                    text: 'Profsaude inativado com sucesso!',
                                    showConfirmButton: true,
                                    timer: 10000
                                }).then(function(){
                                    window.location.href = "${pageContext.request.contextPath}/ProfsaudeListar";
                                })
                            } else {
                                Swal.fire({
                                    position: 'top-end',
                                    icon: 'error',
                                    title: 'Erro',
                                    text: 'Não foi possível inativar profsaude!',
                                    showConfirmButton: true,
                                    timer: 10000
                                }).then(function(){
                                    window.location.href = "${pageContext.request.contextPath}/ProfsaudeListar";
    ...
    function validarCampos() {
        ...
        if (document.getElementById("cartprof").value == '') {
            Swal.fire({
                position: 'center',
                icon: 'error',
                title: 'Verifique a Carteira Profissional do Profsaude!',
                showConfirmButton: true,
                timer: 2000
            });
            $("cartprof").focus();
        } else if (document.getElementById("nomepessoa").value == '') {
            Swal.fire({
                position: 'center',
                icon: 'error',
    ...
    function gravarDados() {
        ...
        $.ajax({
            type: 'post',
            url: 'ProfsaudeCadastrar',
            data: {
                idprofsaude: $('#idprofsaude').val(),
                idpessoa: $('#idpessoa').val(),
                cpfcnpjpessoa: $('#cpfcnpjpessoa').unmask().val(),
                cartprof: $('#cartprof').val(),
                    ...
                    function (data) {
                        ...
                        if (data == 1) {
                            Swal.fire({
                                ...
                                icon: 'success',
                                title: 'Sucesso',
                                text: 'Profsaude gravado com sucesso!',
                                showConfirmButton: true,
                                timer: 20000
                            }).then(function(){
                                window.location.href = "${pageContext.request.contextPath}/ProfsaudeListar";
                            ...
                            Swal.fire({
                                ...
                                icon: 'error',
                                title: 'Erro',
                                text: 'Não foi possível gravar o profsaude!',
                                showConfirmButton: true,
                                timer: 20000
                            }).then(function(){
                                window.location.href = "${pageContext.request.contextPath}/ProfsaudeListar";
