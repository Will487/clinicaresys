...
@WebServlet(name = "AdministradorListar", urlPatterns = {"/AdministradorListar"})
public class AdministradorListar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            request.setAttribute("administradores", dao.listar());
            GenericDAO oEstadoDAO = new EstadoDAO();
            request.setAttribute("estados", oEstadoDAO.listar());
            request.getRequestDispatcher("/cadastros/administrador/administrador.jsp").forward(request, response);
        } catch (Exception ex){
            //ex.printStackTrace();
