 ...
 * @author Aluno
 */
@WebServlet(name = "ProfsaudeCarregar", urlPatterns = {"/ProfsaudeCarregar"})
public class ProfsaudeCarregar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            Profsaude oCli = (Profsaude) dao.carregar(idProfsaude);
            
            Gson ogson = new GsonBuilder().setDateFormat("yyyy-MM-dd").create();
            String jSon = ogson.toJson(oCli);
            response.getWriter().write(jSon);
            
        }catch(Exception ex){
            System.out.println("Erro ao buscar profsaude- "+ex.getMessage());
            ex.printStackTrace();
        }
    }
