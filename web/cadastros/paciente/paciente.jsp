...
<div class="container-fluid">
    <!-- Page Heading -->
    <h1 class="h3 mb-2 text-gray-800">Paciente</h1>
            ...
            <table id="datatable" class="display">
                <thead>
                    <tr>
                        ...
                        <th align="center">Carteirinha</th>
                ...
                <tbody>
                    <c:forEach var="paciente" items="${pacientes}">                    
                        <tr>
                            <td align="center">${paciente.idPaciente}</td>  
                            <td align="center">${paciente.carteirinha}</td>
                            <td align="center">${paciente.nome}</td>
                            <td align="center">${paciente.cpfCnpj}</td>
                       <td align="center">${paciente.cidade.nomeCidade}-${paciente.cidade.estado.siglaEstado}</td>
                            <td align="center">
                                <a href="#modaladicionar" class="btn btn-success adicionar" data-toggle="modal" 
                                   data-id="" onclick="setDadosModal(${paciente.idPaciente})">
                                    <i class="fas fa-plus fa-fw"></i> Alterar </a>
                            </td>
                            <td align="center">
                                <a href="#" onclick="deletar(${paciente.idPaciente})">
                                    <button class="btn 
                                          <c:out value="${paciente.situacao == 'A' ? 'btn-danger': 'btn-success'}"/>">
                                        <i class="fas fa-fw 
                                           <c:out value="${paciente.situacao == 'A' ? 'fa-times' : ' fa-plus'}"/>"></i>
                                        <Strong>
                                            <c:out value="${paciente.situacao == 'A' ? 'Inativar': 'Ativar'}"/>
                                        </Strong>
                                    </button></a>
                            </td>
                        </tr>                
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                                ...
                                <select class="form-control" name="idestado" id="idestado" 
                                    ...
                                    <c:forEach var="estado" items="${estados}">
                                        <option value="${estado.idEstado}"
                                                ${paciente.cidade.estado.idEstado == estado.idEstado ? "selected" : ""}>
                                            ${estado.nomeEstado}
                                        </option>
                                    </c:forEach>
                                </select>
                                ...
                                <select class="form-control" name="idcidade" id="idcidade" required>
                                    ...
                                    <c:forEach var="cidade" items="${cidades}">
                                        <option value="${cidade.idCidade}"
                                                ${paciente.cidade.idCidade == cidade.idCidade ? "selected" : ""}>
                                            ${cidade.nomeCidade} 
                                        </option>
                                    </c:forEach>
                                </select>
                        ...
                        <div class="form-line row">
                            ...
                            <div class="col-sm">
                                ...
                                <select class="form-control" name="permitelogin" id="permitelogin">
                                    <option value="N" ${paciente.permiteLogin == 'N' ? "selected" : ""}>Não</option>
                                    <option value="S" ${paciente.permiteLogin == 'S' ? "selected" : ""}>Sim</option>       
                                </select>    
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" type="text" name="carteirinha" id="carteirinha"/>
                    </div>
...
<script>
    ...
    function setDadosModal(valor) {
        ...
        var idCli = valor;
        if (idCli != "0"){
            //existe paciente para buscar (alteração)
            $.getJSON('PacienteCarregar', {idPaciente: idCli}, function(respostaServlet){
                console.log(respostaServlet);
                var id = respostaServlet.idPaciente;
                if(id != "0"){
                    $('#idpessoa').val(respostaServlet.idPessoa);
                    $('#idpaciente').val(respostaServlet.idPaciente);
                    $('#situacao').val(respostaServlet.situacao);
                    $('#carteirinha').val(respostaServlet.carteirinha);
    ...
    function carregarPessoa(v) {
        ...
        $(document).ready(function () {
            $.getJSON('PessoaBuscarCpfCnpj', {cpfcnpjpessoa: idM, tipopessoa: tipoPessoa}, function (respostaCli) {
                ...
                //var id = respostaCli.idPaciente;
                if (respostaCli != null)
                {
                    $('#idpaciente').val(respostaCli.idPaciente);
                    $('#permitelogin').val(respostaCli.permiteLogin);
                    $('#situacao').val(respostaCli.situacao);
                    $('#carteirinha').val(respostaCli.carteirinha);
                    ...
                    //se não encontrou paciente busca por pessoa somente
                    tipoPessoa = 'pessoa';
    ...
    function deletar(codigo){
        ...
        Swal.fire({
            ...
            text: "Você deseja realmente inativar/ativar o paciente?",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'post',
                    url: '${pageContext.request.contextPath}/PacienteExcluir',
                    data:{
                        idPaciente: id
                    },
                    success:
                        function(data){
                            if(data == 1){
                                Swal.fire({
                                    ...
                                    window.location.href = "${pageContext.request.contextPath}/PacienteListar";
                                })
                            } else {
                                Swal.fire({
                                    position: 'top-end',
                                    icon: 'error',
                                    title: 'Erro',
                                    text: 'Não foi possível inativar paciente!',
                                    showConfirmButton: true,
                                    timer: 10000
                                }).then(function(){
                                    window.location.href = "${pageContext.request.contextPath}/PacienteListar";
    ...
    function validarCampos() {
        ...
        if (document.getElementById("carteirinha").value == '') {
            Swal.fire({
                position: 'center',
                icon: 'error',
                title: 'Verifique a Observação do Paciente!',
                showConfirmButton: true,
                timer: 2000
            });
            $("carteirinha").focus();
                    ...
                    function (data) {
                        ...
                        if (data == 1) {
                            Swal.fire({
                                ...
                                icon: 'success',
                                title: 'Sucesso',
                                text: 'Paciente gravado com sucesso!',
                                showConfirmButton: true,
                                timer: 20000
                            }).then(function(){
                                window.location.href = "${pageContext.request.contextPath}/PacienteListar";
                            ...
                            Swal.fire({
                                ...
                                icon: 'error',
                                title: 'Erro',
                                text: 'Não foi possível gravar o paciente!',
                                showConfirmButton: true,
                                timer: 20000
                            }).then(function(){
                                window.location.href = "${pageContext.request.contextPath}/PacienteListar";
